...
from transactions.models import Transaction
from .models import UserBankAccount
from transactions.constants import TRANSFER, DEPOSIT

def send_transfer_email(user, recipient_email, amount, sender_account_no, recipient_account_no):
    """
    Sends a transactional email to notify the sender and recipient of a successful transfer.
    """
    try:
        # Email to the sender
        sender_message = render_to_string('transactions/transfer_sender.html', {
            'user': user,
            'amount': amount,
        ...
        recipient_message = render_to_string('transactions/transfer_recipient.html', {
            'user': recipient_email,  # Assuming the recipient's email is passed as an argument
            'amount': amount,
            'sender_account_no': sender_account_no,
        })
        recipient_email_instance = EmailMultiAlternatives(
            subject='You Received a Transfer',
            body='',
            to=[recipient_email.email]
        )
        recipient_email_instance.attach_alternative(recipient_message, "text/html")
        recipient_email_instance.send()
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if sender_account.balance >= transfer_amount:
                ...
                send_transfer_email(
                    user=request.user,
                    recipient_email=recipient_account.user,
                    amount=transfer_amount,
                    sender_account_no=sender_account.account_no,
                    recipient_account_no=recipient_account_number,
                )
